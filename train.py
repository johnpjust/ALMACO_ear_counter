...
def train(model, optimizer, scheduler, data_loader_train, data_loader_val, data_loader_test, args, empty_logs_train, empty_logs_val, empty_logs_test):
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
        ...
        for ind in range(len(data_loader_val)):
            for i_ in range(3):
                ...
                if i_ == 0:
                    ...
                    validation_loss_empty.append(tf.math.square(count))
                else:
                    validation_loss_static.append(tf.math.square(count))

        validation_loss = tf.reduce_mean(validation_loss)
        validation_loss_empty = tf.reduce_mean(validation_loss_empty)
        validation_loss_static = tf.reduce_mean(validation_loss_static)
        ...
        tf.summary.scalar('loss/validation_empty', validation_loss_empty, globalstep)
        tf.summary.scalar('loss/validation_static', validation_loss_static, globalstep)
        tf.summary.scalar('loss/test', test_loss, globalstep) ##tf.compat.v1.train.get_global_step()
        tf.summary.scalar('loss/test_empty', test_loss_empty, globalstep)  ##tf.compat.v1.train.get_global_step()
        tf.summary.scalar('loss/test_static', test_loss_static, globalstep)  ##tf.compat.v1.train.get_global_step()
